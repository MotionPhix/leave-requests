...
import { ArrowLeft, Save, FileText, Users, Calendar, Shield } from 'lucide-vue-next';
...
const form = useForm({
  ...
  allow_negative_balance: props.leaveType.allow_negative_balance,
});

const submit = () => {
  form.put(route('tenant.management.leave-types.update', {
    tenant_slug: workspace.slug,
    tenant_uuid: workspace.uuid,
    leaveType: props.leaveType.id,
  }));
};
...
<template>
  <TenantLayout>
    <div class="space-y-6 max-w-2xl">
      ...
      <div>
        <h1 class="text-3xl font-bold tracking-tight">Edit Leave Type</h1>
        <p class="text-muted-foreground">
          Modify the details of {{ leaveType.name }}
        </p>
      </div>

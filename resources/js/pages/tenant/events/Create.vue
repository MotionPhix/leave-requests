...
const form = useForm({
  title: '',
  description: '',
  type: (route().params.type as string) || 'announcement',
  location: '',
  color: '#3b82f6',
...
// Methods
function submit() {
  form.post(route('tenant.management.events.store', tenantParams.value))
}
...
<template>
  <TenantLayout>
    <Head :title="`Create Event - ${workspace.name}`" />
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-muted-foreground">
            Add a new event, meeting, or announcement for your team
          </p>
        </div>
